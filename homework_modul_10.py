...
class Save_to_bd(Record):
    def __init__(self, text):
        super().__init__(text)
        cursor.execute("""CREATE TABLE IF NOT EXISTS News (id INTEGER PRIMARY KEY AUTOINCREMENT, text TEXT,
                       city TEXT, date_published TEXT, UNIQUE (text, city)); """)
        cursor.execute("""CREATE TABLE IF NOT EXISTS Private_ad (id INTEGER PRIMARY KEY AUTOINCREMENT, text TEXT,
                       expiration_date TEXT, date_published TEXT, UNIQUE (text, expiration_date)); """)
        cursor.execute("""CREATE TABLE IF NOT EXISTS Tasks (id INTEGER PRIMARY KEY AUTOINCREMENT, text TEXT, [group] TEXT,
                       priority INTEGER, story_points INTEGER, UNIQUE (text, [group], priority, story_points)); """)
        conn.commit()

    def publish_news(self, city):
        cursor.execute("INSERT INTO News (text, city, date_published) VALUES (?, ?, ?)", (self.text, city, self.date_published))
        conn.commit()

    def publish_ad(self, expiration_date):
        cursor.execute("INSERT INTO Private_ad (text, expiration_date, days_left) VALUES (?, ?, ?)", (self.text, expiration_date, (expiration_date - self.date_published.date()).days))
        conn.commit()

    def publish_task(self, group, priority, story_points):
        cursor.execute("INSERT INTO Tasks (text, group, priority, story_points) VALUES (?, ?, ?, ?)", (self.text, group, priority, story_points))
        conn.commit()
...
class Records_from_file_xml(Records_from_file_json):
    def __init__(self, file_path, num_records):
        self.file_path = file_path
        self.num_records = num_records
    ...
    def read_records(self):
        ...
        # Loop through the XML tree and populate the dictionary
        for child in root:
            sub_dict = {}
            for sub_child in child:
                sub_dict[sub_child.tag] = sub_child.text
            my_dict[child.tag] = sub_dict
        self.write_records_to_json(my_dict, file_path)
...
file_path_xml = input("Enter the file's path for rewriting records from xml: ")
if len(file_path_xml) < 1:
    file_path_xml = 'output.xml'
num_records_xml = int(input("Choose how many records from xml file do you want to write in the constant file: "))
r = Records_from_file_xml(file_path_xml, num_records_xml)
records = r.read_records()
r.write_records_to_json(records, output_file)

while True:
    conn = sqlite3.connect("dqe1.db")
    cursor = conn.cursor()
    ...
    if choice == "1":
        ...
        news.publish()
        save_record_to_file(news)
        record = Save_to_bd(text)
        record.publish_news(city)
        ...
        save_record_to_file(private_ad)
        record = Save_to_bd(text)
        record.publish_ad(expiration_date)
        ...
        save_record_to_file(task)
        record = Save_to_bd(text)
        record.publish_task(group, priority, story_points)
